...
if flag_cabozantinib
    renca_bv_coverage = renca_bv_coverage / 5.19; % 5.19, in vivo data from x Luca Cabo.xlsx sheet_name = RENCA, blood vess area
end
...
for i = 1 : size(single_vessel_masks, 2)
    ...
    if bone(row_fin_vess(min_ind), col_fin_vess(min_ind)) ~= site.tumor
        continue
    end
...
if mod(hour, 3) == 0
    ...
    if hour == 3
        area_tumor_t0 = sum(sum(bone == site.tumor));
        tumor_mask_t0 = (bone == site.tumor);
    else
        area_tumor_t1 = sum(sum(bone == site.tumor));
        tumor_mask_t1 = (bone == site.tumor);
    end
    ...
    while ratio < renca_bv_coverage
        ...
        if rand_numb < 0.25 || hour == 3 || area_tumor_t1 <= area_tumor_t0
            ...
            tum_vess_mask = bone == site.tumor | bone == site.vessel;
